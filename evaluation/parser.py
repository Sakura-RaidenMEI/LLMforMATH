...
def run_execute(executor, result, prompt_type, data_name, execute=False):
    ...
    if prompt_type == "self-consistency" and ";" in prediction:
        # split
        answers = [ans.strip() for ans in prediction.split(";") if ans.strip()]     
        # vote for final answer
        frequency = {}
        for ans in answers:
            if ans in frequency:
                frequency[ans] += 1
        ...
        max_count = 0
        final_answer = answers[0]  # 默认选第一个答案（如果所有频率为1）
        for ans, count in frequency.items():
            if count > max_count:
                max_count = count
                final_answer = ans
...
if __name__ == "__main__":
    ...
    prediction="sdialdjwio\\boxed{sda;ssss}"
    prediction=extract_answer(prediction,"math")
    print(prediction)
    prediction=strip_string(prediction,"math")
    print(prediction)
